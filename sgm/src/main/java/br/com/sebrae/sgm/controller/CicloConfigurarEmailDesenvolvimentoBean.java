...
@ConversationScoped
@Named
public class CicloConfigurarEmailDesenvolvimentoBean extends BaseBean {

	private static final long serialVersionUID = 1L;

	private static Logger log = LoggerFactory.getLogger(CicloConfigurarEmailDesenvolvimentoBean.class);
	...
	public void inserirParametro() {
		try {
			if (this.configuracaoEmail.getUnidades() == null || this.configuracaoEmail.getUnidades().isEmpty()) {
				FacesUtil.addErrorMessage("form-configuracao-email:tblUnidadesSelecionadas",
						"Selecione no m\u00EDnimo uma Unidade.");
				return;
			}

			if ((this.configuracaoEmail.getUsuarios() == null || this.configuracaoEmail.getUsuarios().isEmpty())
					&& (this.configuracaoEmail.getUsuarios() == null || this.configuracaoEmail.getUsuarios().isEmpty())) {
				FacesUtil.addErrorMessage("form-configuracao-email:tblUsuariosAdicionados",
						"Selecione no m\u00EDnimo um Usu\u00E1rio");
				return;
			}

			if (this.cicloConfiguracao.getConfiguracoesEmails() == null) {
				this.cicloConfiguracao.setConfiguracoesEmails(new ArrayList<ConfiguracaoEmail>());
			}

			this.cicloConfiguracao.getConfiguracoesEmails().add(this.configuracaoEmail);
	...
	private void verificaStatus() {
		if (this.cicloConfiguracao.getConfiguracoesEmails() != null
			...
			this.cicloConfiguracao.setStatusConfiguracaoEmail(StatusConfiguracao.C);
		} else {
			this.cicloConfiguracao.setStatusConfiguracaoEmail(StatusConfiguracao.A);
		}
	...
	public void excluir() {
		try {
			this.cicloConfiguracao.getConfiguracoesEmails().remove(getConfiguracaoEmail());
			verificaStatus();
	...
	public void alterarParametro() {
		try {
			...
			this.cicloConfiguracao.getConfiguracoesEmails().remove(this.configuracaoEmail);
			this.cicloConfiguracao.getConfiguracoesEmails().add(this.configuracaoEmail);
	...
	public void prepararModalVincularUnidades() {
		try {
			unidadesDisponiveis = unidadeService.findByUf(this.getCicloConfiguracao().getCiclo().getUf());
			unidadesSelecionadasVincular = new ArrayList<Unidade>();
			this.nomeBusca = "";
			editandoUnidades = false;
			if (getConfiguracaoEmail().getUnidades() != null) {
				unidadesDisponiveis.removeAll(getConfiguracaoEmail().getUnidades());
			}
		} catch (Exception e) {
			FacesUtil.addErrorMessage("Erro ao executar a\u00E7\u00E3o, tente novamente mais tarde.");
			log.error(e.getMessage(), e);
	...
	public void vincularUnidades() {
		try {
			if (editandoUnidades) {
				this.getConfiguracaoEmail().setUnidades(unidadesSelecionadasVincular);
			} else {
				if (this.getConfiguracaoEmail().getUnidades() == null) {
					this.getConfiguracaoEmail().setUnidades(new ArrayList<Unidade>());
				}
				List<Unidade> unidadesVinculadosTemp = this.getConfiguracaoEmail().getUnidades();
	...
	public void desvincularUnidade() {
		try {
			this.getConfiguracaoEmail().getUnidades().remove(this.unidadeSelecionada);
		} catch (Exception e) {
			FacesUtil.addErrorMessage("Erro ao executar a\u00E7\u00E3o, tente novamente mais tarde.");
			log.error(e.getMessage(), e);
		}
	...
	public void prepararModalVincularUnidadesEditar() {
		try {
			...
			unidadesSelecionadasVincular = this.configuracaoEmailService
					.getUnidadesConfiguracaoEmail(getConfiguracaoEmail().getId());
			this.nomeBusca = "";
	...
	public void prepararModalVincularUsuarios() {
		try {
			...
			if (this.configuracaoEmail.getPerfis() != null && !this.configuracaoEmail.getPerfis().isEmpty()
					&& this.configuracaoEmail.getUnidades() != null && !this.configuracaoEmail.getUnidades().isEmpty()) {
				...
				if (this.configuracaoEmail.getPerfis() != null && !this.configuracaoEmail.getPerfis().isEmpty()
					...
					this.usuariosDisponiveis.addAll(usuarioService.findByNameLikeAndPerfisAndUnidadesInternos(
							this.nomeBusca, this.configuracaoEmail.getPerfis(), this.configuracaoEmail.getUnidades()));
					this.usuariosDisponiveis.addAll(usuarioService.findByNameLikeAndPerfisAndUnidadesExternos(
							this.nomeBusca, this.configuracaoEmail.getPerfis(), this.configuracaoEmail.getUnidades()));
				}
				...
				if (getConfiguracaoEmail().getUsuarios() != null) {
					usuariosDisponiveis.removeAll(getConfiguracaoEmail().getUsuarios());
				}
			} else {
				this.usuariosDisponiveis = new ArrayList<Usuario>();
				if (this.configuracaoEmail.getPerfis() == null || this.configuracaoEmail.getPerfis().isEmpty()) {
					FacesUtil.addErrorMessage("formUsuario", "Informe o perfil para busca de usu\u00E1rios.");
				} else {
					if (this.configuracaoEmail.getUnidades() == null || this.configuracaoEmail.getUnidades().isEmpty()) {
						FacesUtil.addErrorMessage("formUsuario", "Informe a unidade para busca de usu\u00E1rios.");
					}
				}
			}
		} catch (Exception e) {
	...
	public void buscarUsuariosVincular() {
		try {
			this.usuariosDisponiveis = new ArrayList<Usuario>();
			this.usuariosDisponiveis.addAll(usuarioService.findByNameLikeAndPerfisAndUnidadesInternos(this.nomeBusca,
					this.configuracaoEmail.getPerfis(), this.configuracaoEmail.getUnidades()));
			this.usuariosDisponiveis.addAll(usuarioService.findByNameLikeAndPerfisAndUnidadesExternos(this.nomeBusca,
					this.configuracaoEmail.getPerfis(), this.configuracaoEmail.getUnidades()));
	...
	public void vincularUsuarios() {
		try {
			if (editandoUsuario) {
				this.getConfiguracaoEmail().setUsuarios(this.usuariosSelecionadasVincular);
			} else {
				if (this.getConfiguracaoEmail().getUsuarios() == null) {
					this.getConfiguracaoEmail().setUsuarios(new ArrayList<Usuario>());
				}

				List<Usuario> usuariosVinculadosTemp = this.getConfiguracaoEmail().getUsuarios();
	...
	public void desvincularUsuario() {
		try {
			this.getConfiguracaoEmail().getUsuarios().remove(this.usuarioSelecionado);
		} catch (Exception e) {
			log.error(e.getMessage(), e);
	...
	public void prepararModalVincularUsuarioEditar() {
		try {
			if (this.getConfiguracaoEmail().getPerfis() != null && !this.configuracaoEmail.getPerfis().isEmpty()) {
				this.usuariosDisponiveis = new ArrayList<Usuario>();
				...
				usuariosSelecionadasVincular = configuracaoEmailService
						.getUsuarioConfiguracaoEmail(getConfiguracaoEmail().getId());
				this.nomeBusca = "";
	...
	public List<Perfil> getPerfis() {
		if (perfis == null || perfis.isEmpty()) {
			perfis = perfilService.bindByChaves(Arrays.asList(Perfil.PERFIS_CHEFIA_USUARIO_EXTERNO));
		}
		return perfis;
	}
