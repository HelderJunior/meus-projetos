...
@ConversationScoped
@Named
public class CicloConfigurarExcecaoMetasResultadosBean extends BaseBean {

	private static final long serialVersionUID = 1L;

	private static Logger log = LoggerFactory
			.getLogger(CicloConfigurarExcecaoMetasResultadosBean.class);
	...
	public void inserirParametro() {
		try {
			if (this.getExcecao().getUsuarios() == null
					|| this.getExcecao().getUsuarios().isEmpty()
					&& this.getExcecao().getTipoMeta() == TipoMeta.I) {
				FacesUtil.addErrorMessage(
						"form-metas-monitoramento:tblUsuariosSelecionados",
						"Informe no m\u00EDnimo um Usu\u00E1rio.");
				return;
			}
			if (this.cicloConfiguracao.getExcecoesResponsabilidades() == null) {
				this.cicloConfiguracao
						.setExcecoesResponsabilidades(new ArrayList<ExcecaoResponsabilidades>());
			}
			this.excecao.setTipoExcecao(TipoExcecao.VMR);
			this.cicloConfiguracao.getExcecoesResponsabilidades().add(
					this.excecao);
	...
	private void verificaStatus() {
		if (this.cicloConfiguracao.getExcecoesResponsabilidades() != null
				&& !this.cicloConfiguracao.getExcecoesResponsabilidades()
						.isEmpty()) {
			this.cicloConfiguracao
					.setStatusConfiguracaoExcecaoResponsabilidades(StatusConfiguracao.C);
		} else {
			this.cicloConfiguracao
					.setStatusConfiguracaoExcecaoResponsabilidades(StatusConfiguracao.A);
		}
	...
	public void excluir() {
		try {
			this.cicloConfiguracao.getExcecoesResponsabilidades().remove(
					this.excecao);
			verificaStatus();
	...
	public void prepararModalVincularUsuarios() {
		try {
			if (this.excecao.getUnidade() != null) {
				...
				// usuarioService.findByUnidade(this.excecao.getUnidade());
				...
				FacesUtil.addErrorMessage("formUsuarioMonitoramento",
						"Informe a unidade para busca dos usu\u00E1rios.");
			}
		} catch (Exception e) {
			FacesUtil
					.addErrorMessage("Erro ao executar a\u00E7\u00E3o, tente novamente mais tarde.");
	...
	public void buscarUsuariosVincular() {
		try {
			// this.usuariosDisponiveis =
			// usuarioService.findByNameLikeAndUnidade(this.nomeBusca,
			// this.excecao.getUnidade());
			this.usuariosDisponiveis = usuarioService.findByNameLikeAndTipo(
					this.nomeBusca, TipoUsuario.I);
			if (getExcecao().getUsuarios() != null) {
				usuariosDisponiveis.removeAll(getExcecao().getUsuarios());
			}
		} catch (Exception e) {
	...
	public void vincularUsuarios() {
		try {
			if (editandoUsuario) {
				this.getExcecao()
						.setUsuarios(this.usuariosSelecionadasVincular);
			} else {
				if (this.getExcecao().getUsuarios() == null) {
					this.getExcecao().setUsuarios(new ArrayList<Usuario>());
				}

				List<Usuario> usuariosVinculadosTemp = this.getExcecao()
						.getUsuarios();
	...
	public void prepararModalVincularUsuarioEditar() {
		try {
			...
			usuariosDisponiveis = usuarioService.findAll();
	...
	public List<Usuario> getUsuariosResponsaveis() {
		...
		/*if (this.usuarioResponsaveis == null
				|| this.usuarioResponsaveis.isEmpty()) {
	...
	public List<Unidade> completeUnidade(String query) {
		List<Unidade> retorno = unidadeService.findByDescricaoOrCodigoAndUF(
				query, this.cicloConfiguracao.getCiclo().getUf());
		return retorno;
	}

	public Usuario getResponsavelAtual() {
		if (this.excecao.getUnidade() != null) {
			List<UsuarioPerfil> usuariosPerfil = usuarioPerfilService
					.findByTipoPerfilAndUnidade(Perfil.GERENTE,
							this.excecao.getUnidade());
