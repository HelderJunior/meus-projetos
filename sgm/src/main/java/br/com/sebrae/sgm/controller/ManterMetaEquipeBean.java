...
@ConversationScoped
@Named
public class ManterMetaEquipeBean extends BaseBean {

	private static final long serialVersionUID = 1L;

	private static Logger log = LoggerFactory.getLogger(ManterMetaEquipeBean.class);
	...
	@Inject
	private ListarMetasEquipeBean listarMetasBean;
	...
	public void prepararModalVincularProjetosEditar() {
		try {
			projetosVincular = projetoService.findAllByUf(this.appBean.getUfSelecionada());
			projetosSelecionadosVincular = new ArrayList<Projeto>(meta.getProjetosVinculados());
			this.nomeBusca = "";
	...
	public void vincularUnidades() {
		try {
			if (editandoUnidades) {
				this.meta.setUnidadesVinculadas(unidadesSelecionadasVincular);
			} else {
		...
		if (meta != null) {
			...
			if (cc != null) {
				List<ConfiguracaoMarcoCritico> configuracoesMarcoCritico = configuracaoMarcoCriticoService
						.findByTipoUnidadeCicloConfiguracao(TipoMeta.E, appBean.getUsuarioAutenticado().getUnidade(),
								cc);
	...
	private void validarAdicaoMarcosCriticos() throws ValidateException {
		...
		if (cc != null) {
			List<ConfiguracaoMarcoCritico> configuracoesMarcoCritico = configuracaoMarcoCriticoService
					.findByTipoUnidadeCicloConfiguracao(TipoMeta.E, appBean.getUsuarioAutenticado().getUnidade(), cc);
	...
	public void adicionarMarcoCritico() {
		try {
			...
			log.error(e.getMessage(), e);
			FacesUtil
					.addErrorMessage("formMarcoCritico", "Erro ao adicionar marco critico, tente novamente mais tarde");
		}
	}
	...
	public String enviarAprovacao() {
		try {
			...
			FacesUtil.addErrorMessage("Esta meta n\u00E3o pode ser exclu\u00EDda, pois possui registros associados a ela.");
			log.error(e.getMessage(), e);
		}
		return "";
	}
	...
	public void enviarResposta() {
		try {
			this.metaService.enviarResposta(observacaoSelecionada, meta);
			if (pendenciasBean != null) {
				pendenciasBean.removerMeta(meta);
			}
